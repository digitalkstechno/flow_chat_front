    ...
    return (
        <SuperAdminLayout title="Dashboard">
            <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-foreground tracking-tight">Executive Control Panel</h1>
                    <p className="text-neutral-500 dark:text-neutral-400 mt-1 text-sm">Real-time statistics across multi-tenant database clusters</p>
                </div>
                <button
                    onClick={fetchData}
                    disabled={loading}
                    className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl transition-all text-sm font-medium shadow-[0_4px_20px_rgba(16,185,129,0.2)] active:scale-95 disabled:opacity-50"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="bg-card border border-border rounded-2xl p-6 shadow-xl relative overflow-hidden transition-all duration-300 hover:border-emerald-500/20">
                    ...
                    <div className="mt-1 text-sm font-semibold text-neutral-500 dark:text-neutral-400">Collected Revenue</div>
                    <div className="mt-3 text-xs text-neutral-400 dark:text-neutral-500 flex justify-between">
                        <span>Target: ₹{summary?.totalSubscriptionAmount?.toLocaleString() || 0}</span>
                    </div>
                </div>
                ...
                <div className="bg-card border border-border rounded-2xl p-6 shadow-xl relative overflow-hidden transition-all duration-300 hover:border-amber-500/20">
                    ...
                    <div className="mt-1 text-sm font-semibold text-neutral-500 dark:text-neutral-400">Pending Receivables</div>
                    <div className="mt-3 text-xs text-neutral-400 dark:text-neutral-500">
                        Total uncollected subscription billing
                    </div>
                </div>
                ...
                <div className="bg-card border border-border rounded-2xl p-6 shadow-xl relative overflow-hidden transition-all duration-300 hover:border-indigo-500/20">
                    ...
                    <div className="mt-1 text-sm font-semibold text-neutral-500 dark:text-neutral-400">Total Client Storage Used</div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-8">
                ...
                <div className="xl:col-span-2 bg-card border border-border rounded-2xl shadow-xl overflow-hidden">
                    <div className="p-6 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h2 className="text-lg font-bold text-foreground">Active Client Directory</h2>
                            <p className="text-xs text-neutral-500 mt-1">Real-time status indicators and space quotas</p>
                        </div>
                        <div className="relative max-w-xs w-full">
                            ...
                            <input
                                ...
                                className="w-full bg-neutral-50 dark:bg-neutral-900/50 border border-border rounded-xl py-2 pl-9 pr-4 text-xs focus:ring-2 focus:ring-emerald-500/30 outline-none placeholder:text-neutral-500 text-foreground transition-all"
                            />
                        </div>
                    </div>
                                        ...
                                        <tr key={t._id} className="hover:bg-neutral-50/40 dark:hover:bg-neutral-800/10 transition-colors">
                                            <td className="px-6 py-4">
                                                ...
                                                <div className="text-neutral-400 text-xs mt-0.5 flex items-center gap-1.5">
                                                    ...
                                                    <span className="font-mono text-[10px] text-indigo-500 dark:text-indigo-400">/{t.slug}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                ...
                                {filteredTenants.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-12 text-center text-neutral-500">
                                            No active client directory matching search filters.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-card border border-border rounded-2xl p-6 shadow-xl">
                        <h3 className="font-bold text-foreground text-sm mb-4 uppercase tracking-wider">Tenant Cluster Health</h3>
                        <div className="space-y-3">
                            <div className="flex justify-between items-center text-xs">
                                <span className="font-semibold text-neutral-500 flex items-center gap-2">
                                    <span className="w-2.5 h-2.5 rounded bg-emerald-500" /> Active tenants
                                </span>
                            ...
                            <div className="flex justify-between items-center text-xs">
                                <span className="font-semibold text-neutral-500 flex items-center gap-2">
                                    <span className="w-2.5 h-2.5 rounded bg-red-500" /> Inactive/Suspended
                                </span>
                    ...
                    <div className="bg-card border border-border rounded-2xl p-6 shadow-xl">
                        <h3 className="font-bold text-foreground text-sm mb-4 uppercase tracking-wider">Recent Provisions</h3>
                        <div className="divide-y divide-border/60">
                            ...
                            {(!summary?.recentTenants || summary?.recentTenants.length === 0) && (
                                <p className="text-xs text-neutral-500 text-center py-4">No recent database provisioning activities.</p>
                            )}
                        </div>
                    </div>

                    {/* Top 5 Affiliates by Revenue Card */}
                    <div className="bg-card border border-border rounded-2xl p-6 shadow-xl">
                        <h3 className="font-bold text-foreground text-sm mb-4 uppercase tracking-wider">Top 5 Affiliates by Revenue</h3>
                        <div className="divide-y divide-border/60">
                            {summary?.topAffiliates?.map((aff: any, idx: number) => (
                                <div key={aff._id} className="py-3.5 first:pt-0 last:pb-0 flex items-start justify-between gap-3">
                                    <div className="min-w-0 flex-1">
                                        <div className="font-bold text-foreground text-sm flex items-center gap-2 truncate">
                                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-500 shrink-0">#{idx + 1}</span>
                                            <span className="truncate">{aff.fullName}</span>
                                        </div>
                                        <div className="text-[10px] text-neutral-400 mt-1 flex items-center gap-1.5 truncate">
                                            <span>{aff.clientsCount} {aff.clientsCount === 1 ? 'client' : 'clients'}</span>
                                            <span className="w-1 h-1 rounded-full bg-neutral-600"></span>
                                            <span className="truncate font-mono">{aff.email}</span>
                                        </div>
                                    </div>
                                    <div className="text-right shrink-0">
                                        <div className="font-bold text-emerald-600 dark:text-emerald-400 text-sm">₹{aff.collectedRevenue?.toLocaleString() || 0}</div>
                                        <div className="text-[9px] text-neutral-450 dark:text-neutral-550 mt-0.5">Target: ₹{aff.totalRevenue?.toLocaleString() || 0}</div>
                                    </div>
                                </div>
                            ))}
                            {(!summary?.topAffiliates || summary?.topAffiliates.length === 0) && (
                                <p className="text-xs text-neutral-500 text-center py-4">No affiliate revenue data available.</p>
                            )}
                        </div>
                    </div>
                </div>
