...
export default function SuperAdminTickets() {
    ...
    if (loading) {
        return (
            <SuperAdminLayout title="Support Tickets">
        ...
        <SuperAdminLayout title="Support Tickets">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                    ...
                    <p className="text-neutral-500 dark:text-neutral-400 mt-1 text-sm">Review and resolve system queries raised by tenant administrators</p>
                </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl shadow-xl overflow-hidden backdrop-blur-xl">
                ...
                <div className="p-6 border-b border-border flex flex-col md:flex-row gap-4 items-center justify-between">
                    <div className="relative w-full md:max-w-md">
                        ...
                        <input
                          type="text"
                          placeholder="Search by tenant, subject, description..."
                          value={search}
                            ...
                            {filtered.map((ticket) => (
                                <tr key={ticket._id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/30 transition-colors group">
                                    <td className="px-6 py-4">
                                        ...
                                        {ticket.dealer ? (
                                            <div className="text-[10px] text-amber-600 dark:text-amber-400 mt-1 font-semibold">Dealer: {ticket.dealer.fullName}</div>
                                        ) : (
                                            <div className="text-[10px] text-neutral-400 mt-1 font-semibold">Dealer: Direct</div>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${
                                            ...
                                            <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${ticket.status === 'resolved' ? 'bg-emerald-500' : 'bg-indigo-500'}`} />
                        ...
                        <div className="space-y-4">
                            <div className="bg-neutral-50 dark:bg-neutral-800/50 border border-border rounded-xl p-4 space-y-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <span className="text-[9px] text-neutral-500 uppercase tracking-widest block">Tenant Organization</span>
                                        <span className="text-sm font-semibold text-foreground">{selectedTicket.tenantName} (/{selectedTicket.tenantSlug})</span>
                                        {selectedTicket.dealer && (
                                            <span className="text-[10px] text-amber-600 dark:text-amber-400 block font-semibold">Dealer: {selectedTicket.dealer.fullName} ({selectedTicket.dealer.email})</span>
                                        )}
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
