...
export default function AffiliateLayout({ children, title = 'Affiliate Portal' }: Props) {
    const router = useRouter();
    const [userName, setUserName] = useState('Affiliate');
    ...
    useEffect(() => {
        const verifyAuth = async () => {
            ...
            try {
                ...
                setUserName(user.fullName || 'Affiliate');
                await getMe();
            } catch (err) {
    ...
    const navItems = [
        { label: 'Dashboard', path: '/affiliate/dashboard', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z' },
        { label: 'My Clients', path: '/affiliate/tenants', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' },
        { label: 'Support Tickets', path: '/affiliate/tickets', icon: 'M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z' },
    ];

    return (
        <div className="min-h-screen bg-background font-sans text-foreground transition-colors duration-300">
            <Head>
                <title>{title} | CA Flow Affiliate</title>
            </Head>
            {/* Sidebar */}
            <aside style={{ width: 256 }} className="fixed inset-y-0 left-0 bg-card border-r border-border flex flex-col z-50 transition-all">
                <div className="h-16 flex items-center px-6 border-b border-border">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-amber-600 to-yellow-400 flex items-center justify-center shadow-lg shadow-amber-500/20 mr-3">
                        <span className="font-bold text-white text-xs">CA</span>
                    </div>
                    <span className="font-semibold text-lg text-foreground tracking-tight">Affiliate Portal</span>
                </div>
